require("Coordinates")
local data_saver = require("DataSaver")
print(data_saver)
turtle = turtle
shell = shell

local turtle_name = 'Miner1'
...
CHUNK_LOADER_BLOCKS = 16
CURRENT_CHUNK_LOADER_BLOCKS = 16
...
NOT_ORES = {
    ...
    "quark:limestone",
    "quark:marble", "quark:basalt", "quark:slate"
}
...
function GoToSpecificLayer(build_stars)
    data_saver:SaveData(turtle_name,'Começando a descer pra camada definida, camada definida: ' .. camada_start .. ' camada atual: ' .. COORDS.Y)
    while true do
        local status = IsInDeterminedRange()
        
        if status == true then
            data_saver:SaveData(turtle_name,'Chegou na camada definida, camada definida: ' .. camada_start .. ' camada atual: ' .. COORDS.Y)
...
function VerifyAround(place_to_look, should_return)
    ...
    if not place_to_look.NORTH then
        ...
        if success and IsOre(block) then
            ...
            data_saver:SaveData(turtle_name,'Encontrou minério: '.. block.name)
            MineAround()
            return true
        end
...
function PlaceSideBlock()
    ...
    if not success then
        ...
        turtle.place()
    elseif block.name ~= 'minecraft:wall_torch' and block.name ~= 'chunk:loader' then
        print("Colocando bloco lateral")
        turtle.place()
    end
end
...
function SetupChunkLoader(is_first_time)
    ...
    for i=1, 16 do
        ...
        if not slot then
            print("Sem item")
        elseif slot.name == 'chunk:loader' then
            turtle.select(i)
            break
        end
    end
    COORDS.turnToDirection(COORDS.DIRECTIONS.EAST)
    --dig until theres no block cuz can have sand or gravel
    while true do
        local success, block = turtle.inspect()
        if not success then
            break
        end
        if block.name == 'minecraft:air' or block.name == 'minecraft:water' or block.name == 'minecraft:lava' or block.name == 'minecraft:flowing_water' or block.name == 'minecraft:flowing_lava' then
            break
        end
        turtle.dig()
    end
...
function StripMine()
    ...
    while true do
        ...
        while true do
            ...
            data_saver:SaveData(turtle_name,'Cavando bloco: ' .. block.name)
            if block.name == 'minecraft:air' or block.name == 'minecraft:water' or block.name == 'minecraft:lava' or block.name == 'minecraft:flowing_water' or block.name == 'minecraft:flowing_lava' then
                break
            end
            turtle.dig()
...
elseif chunk_loader_slot.name ~= "chunk:loader" then
    print()
